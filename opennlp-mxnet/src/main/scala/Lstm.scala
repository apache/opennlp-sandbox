...
object Lstm {
  ...
  def lstmUnroll(numLstmLayer: Int, seqLen: Int, numHidden: Int,
                 numLabel: Int, dropout: Float = 0f): Symbol = {
    ...
    val wordvec = Symbol.SliceChannel()()(
      Map("data" -> data, "num_outputs" -> seqLen, "squeeze_axis" -> 1))
  ...
  def lstmInferenceSymbol(numLstmLayer: Int, numHidden: Int,
                          numLabel: Int, dropout: Float = 0f): Symbol = {
    val seqIdx = 0
    val embedWeight = Symbol.Variable("embed_weight")
    val clsWeight = Symbol.Variable("cls_weight")
    ...
    var hidden = data // Symbol.Embedding("embed")()(Map("data" -> data, "input_dim" -> inputSize,
                      //                       "weight" -> embedWeight, "output_dim" -> numEmbed))
