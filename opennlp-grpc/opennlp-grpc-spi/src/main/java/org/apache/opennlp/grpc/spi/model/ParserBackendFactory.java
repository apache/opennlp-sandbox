...
/**
 * Service provider interface for constituency-parsing backends. Each factory parses its own slice
 * of the server configuration and contributes the parsers it finds; the servers parser registry
 * discovers factories through {@link java.util.ServiceLoader} and groups their parsers by id, so
 * several engines can serve the same parser id.
 */
public interface ParserBackendFactory {
  ...
  /**
   * Creates the parsers this backend is configured for.
   *
