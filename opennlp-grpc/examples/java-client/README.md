...
The Java twin of the [Python quickstart](../python-client/README.md). It is a
standalone Maven project that depends on the generated blocking stubs from
`opennlp-grpc-api`, so build the repository first (`mvn clean install` from
`opennlp-grpc/`) to place that artifact in the local Maven repository. JDK 21
...
```bash
mvn -q compile exec:java -Dexec.args="--cleanup"
```

Options (all passed through `-Dexec.args`):
...
Search is not performed in the client process. The client sends documents and
