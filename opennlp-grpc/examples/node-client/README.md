...
The Node.js twin of the [Python quickstart](../python-client/README.md). The v1
protos load at runtime through `@grpc/proto-loader` from
`../../opennlp-grpc-api/src/main/proto`, so there is no code generation step.
...
```bash
npm install
node analyze_and_search.mjs --cleanup
```
...
Search is not performed in the Node.js process. The client sends documents and
