 ...
 * fixture corpus so the live NER integration test is hermetic: no model is downloaded
 * and no third-party model provenance enters the build. The trained model is written to
 * disk and handed to the spawned server via {@code model.name_finder.person.path}.
 */
final class TinyNerModel {
  ...
  static Path trainPersonModel(Path target) throws IOException {
